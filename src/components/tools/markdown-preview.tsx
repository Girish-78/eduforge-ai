...
import { isLikelyHtml } from "@/lib/generated-document";
...
export function MarkdownPreview({ content, contentId, header }: MarkdownPreviewProps) {
  const renderAsHtml = useMemo(() => isLikelyHtml(content), [content]);
        ...
        {renderAsHtml ? (
          ...
          <ReactMarkdown
            ...
            components={{
              ...
              p: ({ children }) => (
                <p className="mb-3 text-slate-800 leading-[1.68]">
                  {formatScientificContent(children)}
                </p>
              ),
              ...
              li: ({ children }) => (
                <li className="leading-[1.65]">{formatScientificContent(children)}</li>
              ),
              pre: ({ children }) => (
              ...
              table: ({ children }) => (
                <div className="pdf-table-wrapper my-4 overflow-x-auto rounded-xl border border-slate-300 bg-white">
                  <table className="w-full table-fixed border-collapse text-[0.96rem] leading-[1.6]">
                    {children}
                  </table>
                </div>
              ),
